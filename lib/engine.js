...
//
// Skill window: games keep a tight level band. Advance never seats with Beginner; the band
// widens step-by-step the longer the front player has waited (like MMR search expansion).
...
// Skill window: a game's level spread (max - min) must stay within 'allowedSpread',
// which starts tight and widens the longer the front player has waited.
const SPREAD_BASE = 2; // Advance(5) may play down to Intermediate(3), never Beginner(1-2)
const SPREAD_MAX = 4;
const RELAX_MS = 120000; // widen the window one tier per 2 minutes of waiting

// FIFO-first: anchor is always the front unit; complete the four with the
// earliest-queued compatible players inside the skill window. Fairness is by
// PLAYER queue position, and rotation memory breaks ties toward fresh partners.
export function pickFour(pool, history, round, opts) {
  ...
  for (let ai = 0; ai < units.length; ai++) {
    ...
    const anchorQt = Math.min.apply(null, locked.map((p) => p.qt));
    const waited = now - anchorQt;
    const allowedSpread = Math.min(
      SPREAD_MAX,
      SPREAD_BASE + Math.floor(waited / RELAX_MS)
    );
    ...
    unitCombos(window, need).forEach((c) => {
      ...
      // FIFO dominates; then tighter skill; then team balance + rotation memory.
      const score = cost * 100000 + spread * 1000 + ta.diff * 10 + rep * 20;
