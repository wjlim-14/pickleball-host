// Pure queue-engine helpers. Operate on plain player objects:
// { id, name, gender:'M'|'F', level:1..5, freq:'any'|'mixed'|'mens'|'womens', qt, status, stackId }
//
// Rotation memory: pass 'history' (a map keyed by "minId-maxId" -> { p: round, o: round }
// recording the last round two players were partners (p) or opponents (o)) and the current
// 'round' number. Repeats are avoided as a SOFT tiebreak — FIFO order still dominates.
//
// Skill rule (strict): Advance (level 4-5) never shares a game with Beginner (level 1-2).
// Intermediate (3) bridges both. Among legal games, tighter level spread and proper
// formats (Men's/Women's/Mixed over 3-1 "open") are preferred.

export const PL = [
  "",
  "Beginner",
  "Beginner–Intermediate",
  "Intermediate",
  "Intermediate–Advance",
  "Advance",
];

export function mainLabel(l) {
  return l <= 2 ? "Beginner" : l === 3 ? "Intermediate" : "Advance";
}

export function buildUnits(list) {
  const seen = {};
  const u = [];
  list.forEach((p) => {
    if (seen[p.id]) return;
    if (p.stackId) {
      const m = list.find((q) => q.stackId === p.stackId && q.id !== p.id);
      if (m) {
        seen[p.id] = seen[m.id] = 1;
        u.push({ players: [p, m], qt: Math.max(p.qt, m.qt) });
        return;
      }
    }
    seen[p.id] = 1;
    u.push({ players: [p], qt: p.qt });
  });
  u.sort((a, b) => a.qt - b.qt);
  return u;
}

export function deriveType(four) {
  const m = four.filter((p) => p.gender === "M").length;
  if (m === 4) return "mens";
  if (m === 0) return "womens";
  if (m === 2) return "mixed";
  return "open"; // 3-1 or 1-3 — allowed when everyone in the four is flexible
}

export function compat(p, type) {
  return p.freq === "any" || p.freq === type;
}

// A foursome is legal if its gender split satisfies every LOCKED player.
// 'any' players impose no constraint, so 3-1 splits are fine among flexible
// players (keeps courts full). Strict 4M / 4W / 2M2F is enforced only when a
// player has locked mens / womens / mixed.
export function gameLegal(four) {
  const m = four.filter((p) => p.gender === "M").length;
  const f = 4 - m;
  let needMens = false,
    needWomens = false,
    needMixed = false;
  four.forEach((p) => {
    if (p.freq === "mens") needMens = true;
    else if (p.freq === "womens") needWomens = true;
    else if (p.freq === "mixed") needMixed = true;
  });
  if (needMens && needWomens) return false; // conflicting locks
  if (needMens && m !== 4) return false;
  if (needWomens && f !== 4) return false;
  if (needMixed && !(m === 2 && f === 2)) return false;
  return true;
}

// Strict skill rule: a game must never contain both a Beginner (level 1-2) and an
// Advance (level 4-5) player. Advance plays with Intermediate-and-up only; Beginner
// plays with Intermediate-and-down. Intermediate (3) bridges both. No time relaxation.
export function bucketOk(four) {
  const lv = four.map((p) => p.level);
  const hasBeg = lv.some((l) => l <= 2);
  const hasAdv = lv.some((l) => l >= 4);
  return !(hasBeg && hasAdv);
}

function keepStack(o) {
  const a = {},
    b = {};
  o[0].forEach((p) => {
    if (p.stackId) a[p.stackId] = 1;
  });
  o[1].forEach((p) => {
    if (p.stackId) b[p.stackId] = 1;
  });
  for (const k in a) if (b[k]) return false;
  return true;
}

const DECAY = 4; // rounds over which a repeat still counts

function pairKey(a, b) {
  return Math.min(a, b) + "-" + Math.max(a, b);
}

// How "stale" a proposed team split is: recent partners cost more than recent opponents.
function repeatPenalty(teams, history, round) {
  if (!history) return 0;
  let pen = 0;
  const partners = [
    [teams[0][0], teams[0][1]],
    [teams[1][0], teams[1][1]],
  ];
  partners.forEach(([a, b]) => {
    const h = history[pairKey(a.id, b.id)];
    if (h && h.p != null) pen += Math.max(0, DECAY - (round - h.p)) * 3;
  });
  teams[0].forEach((a) =>
    teams[1].forEach((b) => {
      const h = history[pairKey(a.id, b.id)];
      if (h && h.o != null) pen += Math.max(0, DECAY - (round - h.o)) * 1;
    })
  );
  return pen;
}

// Split four players into two teams. Mixed (2M2F) pairs one man + one woman per
// team; every other composition (mens / womens / open 3-1) just tries the three
// 2-v-2 splits and keeps the most level-balanced one.
export function teamAssign(four, type, history, round) {
  let opts;
  if (type === "mixed") {
    const M = four.filter((p) => p.gender === "M");
    const F = four.filter((p) => p.gender === "F");
    opts = [
      [[M[0], F[0]], [M[1], F[1]]],
      [[M[0], F[1]], [M[1], F[0]]],
    ];
  } else {
    const g = four;
    opts = [
      [[g[0], g[1]], [g[2], g[3]]],
      [[g[0], g[2]], [g[1], g[3]]],
      [[g[0], g[3]], [g[1], g[2]]],
    ];
  }
  const valid = opts.filter(keepStack);
  if (valid.length) opts = valid;
  let best = null;
  opts.forEach((o) => {
    const d = Math.abs(
      o[0][0].level + o[0][1].level - (o[1][0].level + o[1][1].level)
    );
    const score = d * 2 + repeatPenalty(o, history, round) * 2; // lower is better
    if (!best || score < best.score) best = { score, d, teams: o };
  });
  return { teams: best.teams, diff: best.d };
}

function unitCombos(units, need) {
  const res = [];
  (function go(s, acc, sum) {
    if (sum === need) {
      res.push(acc.slice());
      return;
    }
    if (sum > need || res.length > 400) return;
    for (let i = s; i < units.length; i++) {
      const u = units[i];
      if (sum + u.players.length > need) continue;
      acc.push(u);
      go(i + 1, acc, sum + u.players.length);
      acc.pop();
    }
  })(0, [], 0);
  return res;
}

// Priority order among valid fours (each tier dominates the next):
//   1. tightest skill spread   2. proper format over 3-1 "open"
//   3. FIFO fairness (queue position)   4. team balance   5. rotation memory
// The anchor (front of queue) is always included, so no one starves — the queue
// just gets grouped by skill first.
const W_SPREAD = 1000000;
const W_OPEN = 100000;
const W_FIFO = 100;

// The anchor is the front unit; complete the four with players that pass the hard
// rules (legal format + no Beginner-with-Advance), scored by the tiers above.
export function pickFour(pool, history, round) {
  const units = buildUnits(pool);
  const rank = new Map();
  pool
    .slice()
    .sort((a, b) => a.qt - b.qt)
    .forEach((p, i) => rank.set(p.id, i));
  for (let ai = 0; ai < units.length; ai++) {
    const anchor = units[ai];
    const locked = anchor.players;
    if (locked.length > 4) continue;
    const need = 4 - locked.length;
    const others = units.slice();
    others.splice(ai, 1);
    const window = others.slice(0, 14);
    let best = null;
    unitCombos(window, need).forEach((c) => {
      const extra = c.reduce((a, u) => a.concat(u.players), []);
      const four = locked.concat(extra);
      if (four.length !== 4) return;
      if (!gameLegal(four)) return; // format locks respected
      if (!bucketOk(four)) return; // never Beginner + Advance in one game
      const levels = four.map((p) => p.level);
      const spread = Math.max.apply(null, levels) - Math.min.apply(null, levels);
      const type = deriveType(four);
      const cost = extra.reduce((s, p) => s + (rank.get(p.id) || 0), 0);
      const ta = teamAssign(four, type, history, round);
      const rep = repeatPenalty(ta.teams, history, round);
      const score =
        spread * W_SPREAD +
        (type === "open" ? W_OPEN : 0) +
        cost * W_FIFO +
        ta.diff * 10 +
        rep;
      if (!best || score < best.score)
        best = { score, four, type, teams: ta.teams };
    });
    if (best) return { four: best.four, type: best.type, teams: best.teams };
  }
  return null;
}

export function findChallengers(waitingList, type) {
  const w = waitingList.slice().sort((a, b) => a.qt - b.qt);
  const firstN = (g, n) => {
    const r = [];
    for (let i = 0; i < w.length && r.length < n; i++) {
      if (w[i].gender === g && compat(w[i], type) && !w[i].stackId) r.push(w[i]);
    }
    return r.length === n ? r : null;
  };
  if (type === "mixed") {
    const m = firstN("M", 1),
      f = firstN("F", 1);
    return m && f ? m.concat(f) : null;
  }
  if (type === "mens") return firstN("M", 2);
  if (type === "womens") return firstN("F", 2);
  // open / fallback: earliest two waiting singles regardless of gender
  const r = [];
  for (let i = 0; i < w.length && r.length < 2; i++) if (!w[i].stackId) r.push(w[i]);
  return r.length === 2 ? r : null;
}
